...
class Attention(nn.Module):
    ...
    def forward(
        ...
        kv_restore: Optional[Tuple[int, int, torch.Tensor, torch.Tensor]] = None,
    ):
        if kv_restore is not None:
            return self._forward_segmented(hidden_states, attention_mask, freqs_cis, kv_restore, optimized_attention)
    ...
    def _forward_segmented(self, hidden_states, attention_mask, freqs_cis, kv_restore, optimized_attention=None):
        # 【图片 KV 缓存】分段 prefill：前缀+后缀正常计算，图片区间 [start, end) 的 K/V 从缓存注入，
        # 只对 text 行做 q/k/v 投影与 attention 查询，跳过图片 token 的全部计算。
        batch_size, seq_length, _ = hidden_states.shape
        start, end, cached_k, cached_v = kv_restore
        text_idx = torch.cat([torch.arange(0, start, device=hidden_states.device),
                              torch.arange(end, seq_length, device=hidden_states.device)])

        h_text = hidden_states[:, text_idx]  # (B, T_text, D)
        xq = self.q_proj(h_text)
        xk_text = self.k_proj(h_text)
        xv_text = self.v_proj(h_text)

        T_text = xq.shape[1]
        xq = xq.view(batch_size, T_text, self.num_heads, self.head_dim).transpose(1, 2)
        xk_text = xk_text.view(batch_size, T_text, self.num_kv_heads, self.head_dim).transpose(1, 2)
        xv_text = xv_text.view(batch_size, T_text, self.num_kv_heads, self.head_dim).transpose(1, 2)
        ...
        xq, xk_text = apply_rope(xq, xk_text, freqs_cis=(cos[:, text_idx], sin[:, text_idx], nsin[:, text_idx]))

        # 完整 KV = 前缀 + 图片缓存 + 后缀（图片区间不计算，直接注入）
        cached_k = cached_k.to(device=xk_text.device, dtype=xk_text.dtype)
        cached_v = cached_v.to(device=xv_text.device, dtype=xv_text.dtype)
        xk = torch.cat([xk_text[:, :, :start], cached_k, xk_text[:, :, start:]], dim=2)
        xv = torch.cat([xv_text[:, :, :start], cached_v, xv_text[:, :, start:]], dim=2)

        if optimized_attention is not None:
            # 优化内核（flash）：text 行 q × 全 KV（cross-attention 语义），q_len < kv_len 支持
            seg_mask = None
            if attention_mask is not None:
                if attention_mask.dim() == 4:
                    seg_mask = attention_mask[:, :, text_idx, :]
                else:  # 2D causal mask (seq, seq)
                    seg_mask = attention_mask[text_idx, :].unsqueeze(0).unsqueeze(0)
            gqa_kwargs = {"enable_gqa": True} if self.num_heads != self.num_kv_heads else {}
            out = optimized_attention(xq, xk, xv, self.num_heads, mask=seg_mask, skip_reshape=True, **gqa_kwargs)
        else:
            # 原生 torch 路径（无优化内核时）
            if self.num_heads != self.num_kv_heads:
                gqa = self.num_heads // self.num_kv_heads
                xk_g = xk.repeat_interleave(gqa, dim=1)
                xv_g = xv.repeat_interleave(gqa, dim=1)
            else:
                xk_g, xv_g = xk, xv
            scores = torch.matmul(xq, xk_g.transpose(2, 3)) * (self.head_dim ** -0.5)
            if attention_mask is not None:
                if attention_mask.dim() == 4:
                    scores = scores + attention_mask[:, :, text_idx, :]
                else:  # 2D causal mask (seq, seq)
                    scores = scores + attention_mask[text_idx, :].unsqueeze(0).unsqueeze(0)
            probs = torch.softmax(scores, dim=-1, dtype=torch.float32).to(xq.dtype)
            out = torch.matmul(probs, xv_g)  # (B, H, T_text, D)
            out = out.transpose(1, 2).contiguous().view(batch_size, T_text, -1)
        out = self.o_proj(out)

        # 图片区间输出用原输入占位（每层都会被再次跳过，不参与计算）
        full = hidden_states.clone()
        full[:, :start] = out[:, :start]
        full[:, end:] = out[:, start:]

        # 返回完整 KV（图片区间为缓存值），供后续 decode 步使用
        return full, (xk, xv, seq_length)
...
class Llama2_(nn.Module):
    ...
    def forward(self, x, attention_mask=None, embeds=None, num_tokens=None, intermediate_output=None, final_layer_norm_intermediate=True,
                ...
                kv_restore=None, save_image_kv=None):
        # kv_restore: (start, end, src) —— 图片区间 [start,end) 的每层 KV + 每层输出 hidden 从缓存注入，
        # 跳过图片 token 的 q/k/v/o/MLP/attention 计算（图 hidden 链独立于文本：因果 + 图在前 + 模板前缀固定）
        # src = 每层缓存文件路径列表（磁盘，逐层惰性加载，峰值内存仅单层 ~70MB）
        #       或 [( (k,v), h ), ...] 内存列表（兼容测试）
        # save_image_kv: (start, end, cache_key) —— prefill 后把图片区间 KV + hidden 保存到 self._saved_image_kv 供调用方落盘
        segmented = False
        seg_start = seg_end = 0
        seg_src = None
        if kv_restore is not None:
            segmented = True
            seg_start, seg_end, seg_src = kv_restore
        save_start = save_end = 0
        save_key = None
        if save_image_kv is not None:
            save_start, save_end, save_key = save_image_kv
        ...
        next_key_values = []
        saved_image_hidden = []
        seg_files_mode = segmented and len(seg_src) > 0 and isinstance(seg_src[0], str)
        prev_h = None
        for i, layer in enumerate(self.layers):
            ...
            # 分段复用：本层 KV/hidden 按需加载（磁盘逐层 / 内存）
            ...
            if segmented:
                if seg_files_mode:
                    ...
                    layer_h = seg_src[i][1]
                # 图区间输入 = 缓存的上层输出（i>0）；i==0 时输入即 embeds 图区间（每次 prefill 相同）
                if i > 0:
                    x[:, seg_start:seg_end] = prev_h.to(dtype=x.dtype, device=x.device)

            past_kv = None
            if past_key_values is not None:
                past_kv = past_key_values[i] if len(past_key_values) > 0 else []

            # 最后一层不跳过：H3 conditioning 需要图区间 hidden 输出，正常计算（图 KV 重算，与缓存位级一致）
            is_last_layer = (i == len(self.layers) - 1)
            layer_kwargs = {}
            if segmented and not is_last_layer:
                layer_kwargs["kv_restore"] = (seg_start, seg_end, layer_k, layer_v)
            x, current_kv = layer(
                x=x,
                attention_mask=mask,
                freqs_cis=freqs_cis,
            ...
            if segmented and not is_last_layer:
                x[:, seg_start:seg_end] = layer_h.to(dtype=x.dtype, device=x.device)
            prev_h = layer_h

            if save_image_kv is not None:
                saved_image_hidden.append(x[:, save_start:save_end].detach().cpu())
        ...
        if save_image_kv is not None and len(next_key_values) > 0:
            saved = [(kv[0][:, :, save_start:save_end].detach().cpu(), kv[1][:, :, save_start:save_end].detach().cpu()) for kv in next_key_values]
            self._saved_image_kv = (save_key, saved, saved_image_hidden)
...
class BaseGenerate:
    ...
    def generate(self, embeds=None, do_sample=True, max_length=256, temperature=1.0, top_k=50, top_p=0.9, min_p=0.0, repetition_penalty=1.0, seed=42, stop_tokens=None, initial_tokens=[], execution_dtype=None, min_tokens=0, presence_penalty=0.0, initial_input_ids=None, position_ids=None, deepstack_embeds=None, visual_pos_masks=None, embeds_info=None, image_kv_restore=None, image_kv_save=None):
        ...
        for step in tqdm(range(max_length), desc="Generating tokens"):
            ...
            if step == 0 and image_kv_restore is not None:
                extra["kv_restore"] = image_kv_restore
            if step == 0 and image_kv_save is not None:
                extra["save_image_kv"] = image_kv_save
